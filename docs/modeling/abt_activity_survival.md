...
ABT por local con target unico de `cese de actividad`: evento por desaparición del `id_local` o por primer cambio robusto `single-single` entre macrocategorias de actividad. El subtipo del evento se conserva solo para auditoría.
...
- Mediana de duración (meses): 135.0
...
- Subtipo auditoría `cambio_actividad`: 21,906
- Subtipo auditoría `desaparición`: 767
- Censurados: 181,197
...
- Filas de division remapeadas por descripcion canónica: 98
- Filas de division marcadas como placeholder/no codificadas: 1,173,250
- Filas de division descartadas por código no valido: 0
...
- Auditoría de cambio: `previous_division_*`, `successor_division_*`, `previous_epigrafe_*`, `successor_epigrafe_*`, `previous_macro_category_*`, `successor_macro_category_*`
...
## Cambios de macrocategoria de actividad más frecuentes tratados como cierre
