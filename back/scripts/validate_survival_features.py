...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--abt-csv", type=Path, default=PROJECT_ROOT / "storage" / "data" / "features" / "activity_survival_abt.csv")
...
def main() -> int:
    ...
    result = validate_survival_feature_frame(
        ...
        metrics_json=_tag_path(PROJECT_ROOT / "storage" / "models" / "survival_feature_validation.json", args.output_tag),
        report_md=_tag_path(PROJECT_ROOT / "docs" / "modeling" / "survival_feature_validation.md", args.output_tag),
    )
