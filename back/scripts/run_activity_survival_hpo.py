...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rolling-baseline-json", type=Path, default=PROJECT_ROOT / "storage" / "models" / "activity_survival_rolling_backtest.json")
    parser.add_argument(
        "--progress-file",
        type=Path,
        default=PROJECT_ROOT / "storage" / "models" / "run_progress_activity_survival_hpo.json",
    )
...
def main() -> int:
    ...
    result = run_activity_survival_hpo(
        ...
        metrics_json=_tag_path(PROJECT_ROOT / "storage" / "models" / "activity_survival_hpo.json", args.output_tag),
        report_md=_tag_path(PROJECT_ROOT / "docs" / "modeling" / "activity_survival_hpo.md", args.output_tag),
        checkpoint_json=_tag_path(PROJECT_ROOT / "storage" / "models" / "activity_survival_hpo_checkpoint.json", args.output_tag),
