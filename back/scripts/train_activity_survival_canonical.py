...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--progress-file",
        type=Path,
        default=PROJECT_ROOT / "storage" / "models" / "run_progress_survival_activity_canonical.json",
    )
...
def main() -> int:
    ...
    result = train_canonical_survival_models(
        abt_csv=PROJECT_ROOT / "storage" / "data" / "features" / "activity_survival_abt.csv",
        metrics_json=_tag_path(PROJECT_ROOT / "storage" / "models" / "survival_activity_canonical_metrics.json", args.output_tag),
        report_md=_tag_path(PROJECT_ROOT / "docs" / "modeling" / "survival_activity_canonical.md", args.output_tag),
        map_export_csv=_tag_path(PROJECT_ROOT / "storage" / "data" / "exports" / "activity_survival_map_export.csv", args.output_tag),
