...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hpo-json", type=Path, default=PROJECT_ROOT / "storage" / "models" / "activity_survival_hpo.json")
    parser.add_argument(
        "--progress-file",
        type=Path,
        default=PROJECT_ROOT / "storage" / "models" / "run_progress_activity_survival_cox_ablation.json",
    )
...
def main() -> int:
    ...
    result = run_activity_survival_cox_ablation(
        ...
        metrics_json=_tag_path(PROJECT_ROOT / "storage" / "models" / "activity_survival_cox_ablation.json", args.output_tag),
        report_md=_tag_path(PROJECT_ROOT / "docs" / "modeling" / "activity_survival_cox_ablation.md", args.output_tag),
