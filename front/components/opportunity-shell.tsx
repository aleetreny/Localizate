...
import {
  FALLBACK_OPPORTUNITY_ARTIFACTS,
  FALLBACK_OPPORTUNITY_SECTION_INDEX_ARTIFACTS,
  loadOpportunityArtifactsFromPublic,
...
type OpportunityShellProps = {
  ...
  initialSectionIndex?: OpportunitySectionIndexArtifacts;
};
...
type OpportunityAddressGeocodeSuccessResponse = {
  ok: true;
  result: {
    lat: number;
    lng: number;
...
type OpportunityAddressGeocodeErrorResponse = {
  ok: false;
  error: string;
};

type ManualSelectionSource = "map" | "address" | null;

const IS_PRODUCTION = process.env.NODE_ENV === "production";
const DEV_ARTIFACT_REVALIDATE_MS = 15000;
const EMPTY_SECTION_INDEX = FALLBACK_OPPORTUNITY_SECTION_INDEX_ARTIFACTS;
const SECTION_GEOMETRY_FETCH_CACHE_MODE = process.env.NODE_ENV === "production" ? "force-cache" : "no-store";

export function OpportunityShell({ initialArtifacts, initialSectionIndex }: OpportunityShellProps) {
  ...
  const [selectedListingFocusNonce, setSelectedListingFocusNonce] = useState(0);
  const [manualSelection, setManualSelection] = useState<OpportunityManualSelection | null>(null);
  const [activeMetricId, setActiveMetricId] = useState<string | null>(null);
  const [sectionIndex, setSectionIndex] = useState(initialSectionIndex ?? EMPTY_SECTION_INDEX);
  const [sectionGeometry, setSectionGeometry] = useState<OpportunitySectionFeatureCollection | null>(null);
  const [isLoadingSectionGeometry, setIsLoadingSectionGeometry] = useState(true);
  const [sectionGeometryError, setSectionGeometryError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function syncArtifacts(resetManualSelection: boolean) {
      ...
      if (resetManualSelection) {
        ...
        setManualSelectionSource(null);
        setResolvedAddressLabel(null);
        setAddressSearchState({ status: "idle", message: null });
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadSectionIndex() {
      const payload = await loadOpportunitySectionIndexFromPublic();
      if (alive) {
        setSectionIndex(payload);
      }
    }

    if (initialSectionIndex && artifacts.meta.section_index_path === normalizedInitialArtifacts.meta.section_index_path) {
      setSectionIndex(initialSectionIndex);
    } else {
      setSectionIndex(EMPTY_SECTION_INDEX);
    ...
    artifacts.meta.section_index_path,
    initialSectionIndex,
    normalizedInitialArtifacts.meta.section_index_path,
  ]);

  useEffect(() => {
    let alive = true;

    async function loadSectionGeometry() {
      try {
        const response = await fetch(artifacts.meta.section_geojson_path, { cache: SECTION_GEOMETRY_FETCH_CACHE_MODE });
        if (!response.ok) {
        ...
        setSectionGeometry(payload);
        setSectionGeometryError(null);
      } catch (error) {
        if (!alive) {
          return;
        }
        setSectionGeometry(null);
        setSectionGeometryError(error instanceof Error ? error.message : "No se pudo cargar la geometría");
      } finally {
        if (alive) {
          setIsLoadingSectionGeometry(false);
        }
      }
    }
    ...
    setSectionGeometry(null);
    setSectionGeometryError(null);
    setIsLoadingSectionGeometry(true);
    void loadSectionGeometry();
  ...
  const selectedSectionKey = normalizeSectionKey(manualSelection?.section.section_key ?? selectedListing?.section_key ?? null);

  const allSections = useMemo(() => {
    return sectionIndex.sections;
  }, [sectionIndex.sections]);

  const sectionsByKey = useMemo(() => {
    const entries: Array<[string, OpportunitySection]> = [];
    for (const section of allSections) {
      const normalizedKey = normalizeSectionKey(section.section_key);
      if (!normalizedKey) {
        continue;
      }
      entries.push([normalizedKey, section]);
    }
  ...
  const detailMetrics = useMemo(() => {
    if (manualSelection) {
      ...
      return [
        ...buildSectionPrimaryMetrics(section, opportunityBenchmarks),
        ...buildContextMetrics({
    ...
    return [
      ...buildPointPrimaryMetrics(selectedListing, opportunityBenchmarks),
      ...buildContextMetrics({
  ...
  }, [manualSelection, opportunityBenchmarks, selectedListing]);
  ...
  const activeStats = useMemo(() => {
    const survivalValues = filteredPoints.map((point) => getPointSurvival(point)).filter(isFiniteNumber);
  ...
  const isAddressSearchLoading = addressSearchState.status === "loading";

  function clearAddressSearchFeedback() {
    setManualSelectionSource(null);
    setResolvedAddressLabel(null);
    setAddressSearchState({ status: "idle", message: null });
  }

  async function handleAddressSearchSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (query.length < 4) {
      setAddressSearchState({
        status: "error",
        message: "Asegúrate de indicar la calle y el número para ubicarla con precisión.",
      });
      return;
    }

    if (isLoadingSectionGeometry) {
      setAddressSearchState({
        status: "error",
        message: "Estamos cargando la geometría del mapa. Vuelve a intentar en un momento.",
      });
      return;
    }
    ...
    if (!sectionGeometry) {
      setAddressSearchState({
        status: "error",
        message: sectionGeometryError
          ? `No se ha podido cargar la geometría de secciones (${sectionGeometryError}).`
          : "No hay geometría disponible para resolver la dirección en el mapa.",
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !isOpportunityAddressGeocodeSuccessResponse(payload)) {
        const errorPayload = isOpportunityAddressGeocodeErrorResponse(payload) ? payload : null;
        setAddressSearchState({
          status: "error",
          message: errorPayload?.error ?? "No se ha podido localizar la dirección.",
        });
        return;
      }

      const matchedSection = resolveSectionForCoordinate(
        sectionGeometry,
        payload.result.lng,
        payload.result.lat,
        sectionsByKey,
      ...
      if (!matchedSection) {
        setAddressSearchState({
          status: "error",
          message: "La dirección cae fuera del área analizada de Madrid o no tiene sección censal asignable.",
        });
        return;
      }
      ...
      setManualSelectionSource("address");
      setResolvedAddressLabel(payload.result.display_name);
      setAddressSearchState({
        status: "success",
        message: payload.result.number_match
      ...
      setAddressSearchState({
        status: "error",
        message: "La búsqueda falló por un problema de red. Inténtalo de nuevo.",
      });
    }
  }
    ...
    <main className="app-shell">
      <aside className="sidebar panel">
        ...
        <div className="stat-grid">
          ...
          <div className="stat-card">
            <span className="label">Supervivencia Mediana 24m</span>
        ...
        <section className="address-search-card">
          ...
          <form className="address-search-form" onSubmit={handleAddressSearchSubmit}>
            <label className="address-search-label" htmlFor="opportunity-address-query">
              Dirección en Madrid
            </label>
            <input
              autoComplete="street-address"
              className="address-search-input"
              id="opportunity-address-query"
              onChange={(event) => setAddressQuery(event.target.value)}
              placeholder="Ejemplo: Calle de Bravo Murillo 120"
              value={addressQuery}
            />
            <button
              className="sidebar-primary-action"
              disabled={isAddressSearchLoading || isLoadingSectionGeometry || addressQuery.trim().length === 0}
              type="submit"
            >
              {isAddressSearchLoading ? "Buscando dirección..." : "Ubicar dirección en el mapa"}
            </button>
          </form>

          {addressSearchState.message ? (
            <p className="address-search-feedback" data-status={addressSearchState.status}>
              {addressSearchState.message}
            </p>
          ) : (
            <p className="address-search-feedback" data-status="idle">
              Te llevaremos al punto, resaltaremos la sección y abriremos su lectura contextual igual que al hacer clic en el mapa.
            </p>
          )}

          {resolvedAddressLabel ? (
            <p className="address-search-match">Ultima dirección localizada: {resolvedAddressLabel}</p>
          ) : null}
        </section>
        ...
        <section className="detail-card">
          {manualSelection ? (
            <ManualPointDetail
              ...
              resolvedAddressLabel={resolvedAddressLabel}
...
function ManualPointDetail({
  ...
  return (
    <>
      ...
      {resolvedAddressLabel ? <p className="detail-subtle">Dirección buscada: {resolvedAddressLabel}</p> : null}
      <p className="detail-subtle">El punto se interpreta con la zona pequeña del censo donde cae, no como una dirección inventada.</p>
      <div className="detail-meta">
        <HeaderMetricChip activeMetricId={activeMetricId} metric={headerMetrics.opportunityTier} onMetricSelect={onMetricSelect} />
        <HeaderMetricChip activeMetricId={activeMetricId} metric={headerMetrics.riskPercentile} onMetricSelect={onMetricSelect} />
      </div>
...
function getSectionSurvival(section: OpportunitySection) {
  return section.expected_survival_24m;
}

function getOpportunityArtifactSignature(artifacts: OpportunityArtifacts) {
  return [
    artifacts.meta.generated_at,
    artifacts.meta.section_index_path,
    artifacts.meta.section_geojson_path,
    artifacts.points.length,
  ].join(":");
}
...
function formatRiskIndexValue(value: number | null) {
  if (!isFiniteNumber(value)) {
    return "Sin datos";
  }
...
function buildPointPrimaryMetrics(point: OpportunityPoint, benchmarks: OpportunityBenchmarkContext | null): MetricDefinition[] {
  return [
    {
      id: `listing:${point.listing_id}:survival:24m`,
      label: "Supervivencia 24m",
      value: formatPercent(getPointSurvival(point)),
      summary: "Es la probabilidad esperada de que un local comparable siga activo dentro de 24 meses en este entorno.",
      calculation: "Partimos de la puntuación de riesgo del modelo para la sección donde cae el local y la transformamos en supervivencia esperada para el horizonte fijo de 24 meses."
    },
    buildRankingMetric({
...
function buildSectionPrimaryMetrics(section: OpportunitySection, benchmarks: OpportunityBenchmarkContext | null): MetricDefinition[] {
  return [
    {
      id: `section:${section.section_key}:survival:24m`,
      label: "Supervivencia 24m",
      value: formatPercent(getSectionSurvival(section)),
      summary: "Es la probabilidad esperada de que un local comparable siga activo dentro de 24 meses en esta sección.",
      calculation: "Partimos de la puntuación de riesgo agregada de la sección y la transformamos en supervivencia esperada para el horizonte fijo de 24 meses."
    },
    buildRankingMetric({
      scopeId: `section:${section.section_key}`,
...
function resolveRiskIndexDisplay(
  sectionRiskIndex: number | null,
  benchmark: OpportunityFieldInsight | null | undefined,
): { value: number | null; source: RiskIndexDisplaySource } {
  const sectionValue = isFiniteNumber(sectionRiskIndex) ? Number(sectionRiskIndex) : null;

  if (isFiniteNumber(sectionValue)) {
    return { value: sectionValue, source: "section" };
  }

  if (isFiniteNumber(benchmark?.barrioMean)) {
    return { value: benchmark.barrioMean, source: "barrio" };
  }

  if (isFiniteNumber(benchmark?.districtMean)) {
...
function buildRiskIndexSourceLabel(source: RiskIndexDisplaySource) {
  if (source === "barrio") {
    return "barrio";
  }
  if (source === "district") {
...
function buildRiskIndexCalculation(source: RiskIndexDisplaySource) {
  ...
  if (source === "none") {
    return `${baseCalculation} Si faltan sección y referencias territoriales, mostramos Sin datos.`;
  }

  return `${baseCalculation} Si falta el índice en la sección visible, usamos el valor medio del ${buildRiskIndexSourceLabel(source)} para mantener continuidad en la lectura.`;
}
...
function buildMetricWhyUseful(metric: MetricDefinition) {
  if (metric.id.includes(":survival:")) {
    return "Te sirve para comparar de un vistazo qué entornos aguantan mejor una apertura parecida a 24 meses.";
  }
  ...
  if (metric.id.endsWith(":risk-index")) {
    return "Te permite leer de forma rápida la exigencia estructural del entorno y compararla con las medias del barrio, distrito y Madrid.";
  }
...
function isOpportunityAddressGeocodeSuccessResponse(
  payload: unknown,
): payload is OpportunityAddressGeocodeSuccessResponse {
  if (!payload || typeof payload !== "object") {
    return false;
  }

  const candidate = payload as Partial<OpportunityAddressGeocodeSuccessResponse>;
  return candidate.ok === true
    && Boolean(candidate.result)
    && typeof candidate.result?.lat === "number"
    && typeof candidate.result?.lng === "number"
    && typeof candidate.result?.display_name === "string"
    && typeof candidate.result?.number_match === "boolean";
}

function isOpportunityAddressGeocodeErrorResponse(
  payload: unknown,
): payload is OpportunityAddressGeocodeErrorResponse {
  if (!payload || typeof payload !== "object") {
    return false;
  }

  const candidate = payload as Partial<OpportunityAddressGeocodeErrorResponse>;
  return candidate.ok === false && typeof candidate.error === "string";
}

function normalizeSectionKey(value: string | number | null | undefined) {
...
function resolveSectionForCoordinate(
  sectionGeometry: OpportunitySectionFeatureCollection,
  lng: number,
  lat: number,
  sectionsByKey: ReadonlyMap<string, OpportunitySection>,
) {
  for (const feature of sectionGeometry.features) {
    if (!feature.geometry || !feature.properties?.section_key) {
      continue;
    }

    if (!isPointInsideSectionGeometry(lng, lat, feature.geometry.type, feature.geometry.coordinates)) {
      continue;
    }

    const sectionKey = normalizeSectionKey(feature.properties.section_key);
    if (!sectionKey) {
      continue;
    }
...
function isPointInsideSectionGeometry(
  lng: number,
  lat: number,
  ...
  if (geometryType === "Polygon") {
    const polygonRings = coordinates as number[][][];
    return isPointInsidePolygonRings(lng, lat, polygonRings);
  }
...
function isPointOnSegment(
  ...
  // GeoJSON rings are usually closed (first point repeated at the end), which
  // creates a zero-length segment in the scan. Avoid treating every point as
  // collinear with that degenerate edge.
  ...
  const cross = ((pointLat - startLat) * deltaLng) - ((pointLng - startLng) * deltaLat);
  if (Math.abs(cross) > epsilon) {
    return false;
  }

  const dot = ((pointLng - startLng) * deltaLng) + ((pointLat - startLat) * deltaLat);
  if (dot < -epsilon) {
    return false;
  }

  if (dot - segmentLengthSquared > epsilon) {
    return false;
  }
